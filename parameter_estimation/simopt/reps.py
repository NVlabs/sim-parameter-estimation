...
class REPS():
    def __init__(self, mean_init, cov_init, kl_threshold=1.0, covariance_damping=2.0,
                 min_temperature=0.001):

        self.current_mean = mean_init 
        self.current_cov = cov_init 

        self._kl_threshold = kl_threshold
        self._covariance_damping = covariance_damping
        self._min_temperature = min_temperature
        
    def learn(self, samples, costs):
        mean_new = np.zeros(npa(self.current_mean).shape)
        cov_new = np.zeros(npa(self.current_cov).shape)

        inv_cov_new = np.zeros(npa(cov_new).shape)
        chol_cov_new = np.zeros(npa(cov_new).shape)
        ...
        self.current_mean = mean_new
        self.current_cov = cov_new
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        curr_mean, curr_cov = reps_optim.learn(samples, costs)
