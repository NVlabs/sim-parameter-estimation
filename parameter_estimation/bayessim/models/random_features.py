...
class RFF:
    ...
    def __init__(self, m, d, sigma, cosOnly=False, quasiRandom=False,
        ...
        if self.cosOnly:  # cos only features
            self.coeff = self._drawCoeff(rffKernel, m)
            self.offset = 2.0 * np.pi * np.random.rand(1, m)
            ...
            self.coeff = self._drawCoeff(rffKernel, int(m//2))
    ...
    def _toCosOnlyFeatures(self, x):
        ...
        return self.a * np.cos(inner + self.offset)

    def _toCosSinFeatures(self, x):
        inner = x.dot(self.coeff.T)
        return self.a * np.hstack((np.cos(inner), np.sin(inner)))
